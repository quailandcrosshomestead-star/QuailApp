...
//  Fits ONE FC-16 (MAX7219) 4-in-1 LED matrix module  ->  32x8
//  Printed per-pixel grille face | ESP32 lives inside |
//  hangs on two wall screws via integrated KEYHOLE slots
//  Units: millimetres
// =================================================================
//  PARTS (set `part`), each -> its own STL:
//    body           one-piece shell with the pixel-grille face
//    back_cover     rear lid: clamps board + holds ESP32 + KEYHOLES
...
//
//  ASSEMBLY
//    1. Drop the 4-in-1 module face-down into the grille pocket.
//       Wire it up; leads exit the bottom slot and/or the end slots
//       (the module's pin headers tuck into the end slots).
//    2. Bolt the ESP32 to the standoffs on the back cover.
//    3. Screw the cover on - its posts clamp the board to the grille.
...
/* [ RENDER THIS PART ] */
part = "preview_assembled"; // [body, back_cover, grille_test, wall_template, preview_assembled, preview_exploded, preview_back]

/* [ Board / display  -  ONE FC-16 4-in-1 module ] */
board_len    = 128;   // PCB length of a 4-in-1 module (4x 8x8 @ 32 mm)
board_h      = 32;    // matrix height (8 rows @ 4 mm)
board_thick  = 13;    // matrix + PCB + rear chips stack
led_area_len = 128;   // lit area length  (32 cols)
led_area_h   = 32;    // lit area height  (8 rows)

/* [ Pixel grille face ] */
grille_t   = 2.0;
cell_pitch = 4.0;   // LED spacing (128/32 = 4.0)
...
wall_hole_span  = 90;   // horizontal distance between the two keyholes
...
module back_cover(){
  ...
  difference(){
    ...
    // cover-to-body screws (countersunk head pocket on outer face)
    for(p=boss_xy){
...
module wall_template(){
  ...
  difference(){
    translate([0,0,0]) linear_extrude(t) rrect(wall_hole_span+24,OUT_H,corner_r);
...
if(part=="body") body();
  ...
  color("SteelBlue") body();
  color("DimGray") translate([0,0,45]) back_cover();
}
else if(part=="preview_back"){
  // look at the rear so the keyholes are visible
